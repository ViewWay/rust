...
impl<'tcx> TyCtxt<'tcx> {
    ...
    /// Feeds the child of lowered AST owner during AST -> HIR delayed lowering.
    pub fn feed_delayed_owner_child(self, key: LocalDefId, owner: MaybeOwner<'tcx>) {
        TyCtxtFeed { tcx: self, key }.delayed_owner_child(owner);
    }
}
