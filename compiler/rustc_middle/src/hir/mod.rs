...
pub struct Crate<'hir> {
    ...
    // Ids of delayed AST owners which are lowered through `lower_to_hir_delayed` query.
    pub delayed_ids: FxIndexSet<LocalDefId>,
    // The resolver and AST crate which are set in the end of the `hir_crate` query
    // and then stolen and dropped in `force_delayed_owners_lowering` query.
...
impl<'hir> Crate<'hir> {
    ...
    pub fn owner(&self, tcx: TyCtxt<'hir>, def_id: LocalDefId) -> MaybeOwner<'hir> {
        ...
        if self.delayed_ids.contains(&def_id) {
            tcx.lower_to_hir_delayed(def_id)
        } else {
            tcx.delayed_owner_child(def_id)
        }
    }
}
