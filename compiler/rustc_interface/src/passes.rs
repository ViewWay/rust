...
pub static DEFAULT_QUERY_PROVIDERS: LazyLock<Providers> = LazyLock::new(|| {
    ...
    providers.queries.lower_to_hir_delayed = rustc_ast_lowering::lower_to_hir_delayed;
    // `delayed_owner_child` is fed during `lower_to_hir_delayed`, by default it returns phantom,
    // as if this query was not fed it means that `MaybeOwner` does not exist for provided DefId.
    providers.queries.delayed_owner_child = |_, _| MaybeOwner::Phantom;
